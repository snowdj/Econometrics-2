...
setlocal indentkeys+==~type,=~interface,=~forall
...
function FortranGetIndent(lnum)
  ...
  if prevstat =~? '^\s*\(\d\+\s\)\=\s*\(else\|case\|where\|elsewhere\|forall\|select\s\+case\)\>'
    ...
    " remove unwanted indent for WHERE and FORALL stmts
    if prevstat =~? '^\s*\(\d\+\s\)\=\s*forall\s*([^()]*\(([^()]*\(([^()]*)[^()]*\)*)[^()]*\)*)\s*\S' 
      let ind = ind - &sw
    endif 
    if prevstat =~? '^\s*\(\d\+\s\)\=\s*where\s*([^()]*\(([^()]*\(([^()]*)[^()]*\)*)[^()]*\)*)\s*\S' 
      let ind = ind - &sw
    endif 
  endif
  ...
  if getline(v:lnum) =~? '^\s*\(\d\+\s\)\=\s*'
	\. '\(else\|elsewhere\|case\|end\s*\(if\|where\|select\|interface\|type\|forall\)\)\>'
    let ind = ind - &sw
  endif
